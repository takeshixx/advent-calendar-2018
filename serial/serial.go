...
func writeConnection(conn net.Conn, challs []chalStruct, messages chan string) {
	...
	if len(challs) == 0{
		...
		for _, chal := range challs {
			...
			//conn.Write([]byte(chal.challenge))
			//fmt.Print(chal.result)
			...
			if int_input == chal.result{
				...
				if correct >= len(challs){
					messages <- "YOU SOLVED IT HAPPY XMAS YAY\n"
					conn.Close()
				}
				...
				return
	                ...
	                //fmt.Print("Received message: ", string(int_input))
...
func main() {
    ...
    fmt.Println("The server2 is listening on Port 8081")
    ...
    for {
	    ...
	    if local_port[1] == "8080"{
		    ...
		    for i:=0; i<= amount; i++ {
			    ...
			    //fmt.Printf("Challenge :%s Result: %d\n", test.challenge, test.result)
		   }
		   log.Printf("Created new Challenges for IP: %s", ip[0])
		   //fmt.Println(ipChallenges)
